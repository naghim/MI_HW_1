...
int main(int argc, char* argv[]) {
	...
	if (mode == 0 || mode == 1 || mode == 2) MODE == mode;
	cout << "Megoldasi modszer: " << MODE << endl;
...
map<VAR, set<VAR>> DomainBuilder(const State& oldState) {
	...
	if (MODE == 3) AC3(homie, D_actual);										  ///AC-3

	D_actual[homie].clear();													  ///Memory optimisation
	return D_actual;
}
